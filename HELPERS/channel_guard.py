"""
PATH: HELPERS/channel_guard.py
TIMESTAMP: 2026-01-01 13:50 EST
VERSION: v2.0.0
DESIGN: Telegram channel subscription verification.
...
class ChannelGuard:
    ...
    async def _scan_once(self) -> Tuple[int, int]:
        async with self._lock:
            ...
            if not events:
                return 0, 0
            # events is list of tuples (event, user_info), sort by event.id
            ...
            for event, user_info in events:
                ...
                # Process event.date - can be datetime or int (timestamp)
    ...
    async def _fetch_leave_events(self) -> List[Tuple[Any, Dict[str, Any]]]:
        ...
        if not client:
            ...
            return []
        # Use same client for resolve_peer so user client can access the channel
    ...
    async def _fetch_recent_activity(self, hours: int = 48, limit: int = 500) -> List[Dict[str, Any]]:
        ...
        for event in getattr(result, "events", []) or []:
            ...
            if isinstance(action, ChannelAdminLogEventActionParticipantLeave):
                ...
                # Check join types, but ChannelAdminLogEventActionParticipantAdd can be None
